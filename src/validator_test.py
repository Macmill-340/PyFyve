...
def validate(result, rules, user_code):
    """Run a single validation rule against execution result. Returns 'Pass' or 'Fail'."""
    ...
    if rules["type"] == "output_check":
        ...
        console.print("❌ Task incomplete — check the task for the required output.", style="error")
        return "Fail"
        ...
        if target_variables.keys() <= user_variables.keys():
            ...
            for key in shared_keys:
                ...
                if target_type is None:
                    console.print(
                        f"❌ Lesson configuration error: Unsupported type '{target_variables[key]}'. Contact developer.",
                        style="error"
                    )
                    return "Fail"
