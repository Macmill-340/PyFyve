        ...
        if system == "Windows":
            ...
            if reset_file or not os.path.exists(filename):
                with open(filename, "w", encoding="utf-8") as f:
                    ...
                    f.write('1. Write your code below.\n')
                    f.write('2. Save the file (Ctrl+S).\n')
                    f.write('3. EXIT this text editor to run your code (CTRL + X).\n')
                    f.write('"""\n\n')
                    if reset_file:
                        f.write(f"{code_seperator}\n")
...
def exec_code(user_code):
    ...
    try:
        ...
        runt_err_map = {
            ...
            "NameError":
                "You're using a variable that doesn't exist yet.\n"
                "Either you haven't created it, or you misspelled it.\n"
                "Remember: names are case-sensitive (age ≠ Age)",
            ...
            "TypeError":
                "You tried to combine incompatible data types (like adding Text + Number).\n"
                "Ensure both values are the same type, or use str() / int() to convert them.",
        }
